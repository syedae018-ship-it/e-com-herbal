      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 sm:space-y-12">
        ...
        <div className="text-center max-w-2xl mx-auto space-y-2">
          ...
          <h2 className="font-serif text-2xl sm:text-3xl lg:text-4xl font-bold text-forest-950">
            {section.heading || 'Why Choose Nutri Life'}
          </h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {section.items.map((reason, index) => {
            ...
            return (
              <div
                key={reason.id || index}
                className="bg-cream-50 p-4 sm:p-6 rounded-2xl border border-sand-200 hover:border-sage-400 hover:bg-cream-100 transition-all duration-300 space-y-3 sm:space-y-4 flex flex-col justify-between"
              >
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-forest-900 text-cream-50 flex items-center justify-center shadow-sm">
                  <Icon className="w-5 h-5 sm:w-6 sm:h-6 text-sage-300" />
                </div>
                <div className="space-y-1 sm:space-y-1.5">
                  <h3 className="font-serif text-xs sm:text-base font-bold text-forest-950">
                    {reason.title}
                  </h3>
                  <p className="text-[10px] sm:text-xs text-charcoal-600 leading-relaxed">
                    {reason.description}
                  </p>
                </div>
              </div>
